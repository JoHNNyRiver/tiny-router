...
var findPath = function(req, res, router, cb) {
  ...
  if (url.indexOf('.') !== -1) {
    if (router.getMime(url).indexOf('image') != -1 || router.getMime(url).indexOf('video') != -1) {
        router.fileExist(source, function(data) {
          if (data) {
            var stat = router.fs.statSync(source)
            var fileSize = stat.size
            res.writeHead(200, {'Content-Type': router.getMime(url), 'Content-Length': fileSize});
            router.fs.ReadStream(source).pipe(res);
          } else {
            res.statusCode = 404;
            res.writeHead(404, {'Content-Type': router.getMime(url), 'Content-Length': 0});
            res.end();
          }
        })
      } else {
